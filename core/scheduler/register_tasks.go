...
func RegisterTasks() {
	tasks := []struct {
		...
		{"0 0/2 * * * *", test.Test1},                 //测试
		{"0 0/10 * * * *", mypackage.FindExpireData1}, //背包数据处理
		{"0 0/2 * * * *", mypackage.FindExpireData2},  //背包数据处理
		{"0 0/2 * * * *", mypackage.FindExpireData3},  //背包数据处理
	}
	//查找mongo表中数据
	go mypackage.FindExpireData1()
	//查inventory_pack_trade_transfer_logs表中数据
	mypackage.FindExpireData2()
	mypackage.FindExpireData3()
